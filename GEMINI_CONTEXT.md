...
-   **Core Data Engine (`surfpy/`)**: This is a powerful, low-level library that acts as the engine for all data fetching and processing. It interfaces directly with external data sources like NOAA buoys and tide stations. `surfpy` is an unmaintained public library — there is a long-term intent to retire it in favor of [`howsit`](https://github.com/mhdietz/howsit), a new intentionally-built, understood replacement, but that migration has not touched the main app yet (see below).

-   **`evaluation/`**: A self-contained research sub-project (own README, own Supabase schema) comparing public wave data sources (NDBC, CDIP buoys) against Surfline's proprietary LOTUS forecast, to answer: can public data + models get "good enough" for surfers without Surfline? Evaluation-only — writes to the `evaluation` schema, invisible to the main app, and does not feed production. See `evaluation/README (1).md` for the full pipeline/roadmap and `evaluation/RESEARCH_NOTES.md` for the underlying wave-science research. `evaluation/` is `howsit`'s proving ground: new fetchers get validated here (against live Surfline ground truth) before the main app or the separate `partiwave` project would ever depend on them. Retiring `surfpy` from *this* main app/`ocean_data/` layer is explicitly not planned yet — only from `evaluation/` and `partiwave` first, since those are far lower-stakes than this production app.
